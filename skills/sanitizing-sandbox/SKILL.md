...
**Staging first-boot:** When writing Pi OS to a stick, generate network config: `./scripts/render-pi-firstboot-network.sh` → `config/generated/pi-firstboot-network.yaml`. User-data: `./scripts/render-pi-firstboot-userdata.sh <hostname>` → `config/generated/pi-firstboot-userdata.yaml`. Copy both to the boot partition after writing the image; set `/etc/hostname` and `/etc/hosts` in rootfs. See **Pi OS image and staging** below.
...
### 5. Pi OS image and staging

Use a scratch folder (e.g. `downloads/`, gitignored) for the image. Raspberry Pi OS Lite 64-bit (Trixie, 2025-12-04):
...
- **Verify:** same directory has `2025-12-04-raspios-trixie-arm64-lite.img.xz.sha256`; run `sha256sum -c` on it.

**Write to USB (whole-disk device, not a partition):**
```bash
xzcat <path-to-image.img.xz> | sudo dd of=/dev/sdX bs=4M status=progress conv=fsync
```
Then: mount boot partition, copy `config/generated/pi-firstboot-network.yaml` → `network-config` and `config/generated/pi-firstboot-userdata.yaml` → `user-data`; mount root partition, set `/etc/hostname` and `/etc/hosts` (127.0.1.1) to the desired hostname.
